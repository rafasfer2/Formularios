aba_familia <- tabPanel(
  ...
  div(
    ...
    p("Preencha os dados de cada membro da família e clique em 'Adicionar Membro' para incluí-lo na lista. O CPF da mulher cadastrada será vinculado automaticamente."),
    
    # Formulário de entrada dos dados familiares
    fluidRow(
      column(6,
             textInput("nome_familiar", labelObrigatorio("Nome completo"), placeholder = "Ex: Maria Silva"),
             radioButtons("sexo_familiar", labelObrigatorio("Sexo"), choices = c("Masculino", "Feminino")),
             radioButtons("parentesco", labelObrigatorio("Parentesco"), choices = c("Filho(a)", "Cônjuge", "Irmão(ã)", "Avô(ó)", "Outro")),
             conditionalPanel(
               condition = "input.parentesco == 'Outro'",
               textInput("parentesco_outros", label = "Informe o parentesco", placeholder = "Ex: Primo(a)")
             )
      ),
      column(6,
             numericInput("idade_familiar", labelObrigatorio("Idade"), value = NA, min = 0),
             selectInput("escolaridade_familiar", labelObrigatorio("Escolaridade"), choices = c(
