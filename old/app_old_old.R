...
ui <- fluidPage(
  ...
  tags$div(
    ...
    div(
      style = "display: flex; flex-wrap: wrap; justify-content: center; gap: 10px; margin: 10px 20px;",
    ...
    div(
      ...
      style = "flex: 1; padding-bottom: 80px;",  # espaço para o rodapé
      fluidRow(
        column(
          9,
          tabsetPanel(
            id = "abas",
            #######################################################################
            ########### Início - Formulário de Cadastro Inicial SEMMU  ############
            tabPanel(
              "Início",
              div(
                class = "capa",
            ...
            ###########  Final - Formulário de Cadastro Inicial SEMMU  ############
            #######################################################################
            ###########     next0 Início - Rede de Atendimento SEMMU   ############
            tabPanel(
              "Rede de Atendimento SEMMU", value = "rede", 
              ...
              conditionalPanel(
                ...
                selectInput("profissional_crm", 
                            labelObrigatorio("Profissional Responsável:"),
                            choices = c("Selecione", "Arlene Ferreira da Cruz Piovezan", "Juliana Pereira Cruz Menezes", "Elane Oliveira Corrêa"))),
              conditionalPanel(
                condition = "input.rede == 'Casa de Mainha'",
                selectInput("profissional_mainha", 
                            labelObrigatorio("Profissional Responsável:"), 
                            choices = c("Selecione", "Daiane Almino Ribeiro", "⁠Elizabeth Rodrigues de Bessa", "Fabiane Lima de Souza","Keylla Alves da Silva"))),
              conditionalPanel(
                condition = "input.rede == 'Casa Abrigo'",
                selectInput("profissional_abrigo", 
                            labelObrigatorio("Profissional Responsável:"), 
                            choices = c("Selecione", "Alba Maria Rodrigues", "⁠Lucinei Aparecida Santos da Luz","⁠Eva Silva de Lima","Natália de Deus")) ),
              conditionalPanel(
                condition = "input.rede == 'SEMMU Até Você'",
                tagList(
                  selectInput("profissional_movel", 
                              labelObrigatorio("Profissional Responsável:"),
                              choices = c("Selecione", "Elisangela Moreira", "Eleusa","Josélia Viana","Sandra Araújo","Keylla Alves da Silva")),
                  selectInput("polo_visitado", 
                              labelObrigatorio("Polo Visitado:"),
                              choices = c("Selecione", "Cedere 1", "Palmares 2", "Valentim Serra", "Paulo Fonteles", "Vila Carimã", "Vila Brasil", "Vila Alto Bonito", "Vila Sansão", "Outros")),
                  conditionalPanel(
            ...
            tabPanel(
              "Dados Iniciais de Cadastro", value = "dados_iniciais",
            ...
            tabPanel(
              "Notificação Individual", value = "notificacao",
            ...
            tabPanel("Dados Familiares", value = "familia",
                     fluidRow(
                       column(4, textInput("nome_familiar", "Nome do Membro", placeholder = "Ex: Maria Silva")),
                       column(4, selectInput("parentesco", "Parentesco", choices = c("", "Filho", "Filha", "Cônjuge", "Irmão", "Irmã", "Outro"))),
                       column(4, selectInput("sexo_familiar", "Sexo", choices = c("", "Masculino", "Feminino", "Outro")))
                     ),
                     
                     fluidRow(
                       column(4, numericInput("idade_familiar", "Idade", value = NA, min = 0)),
                       column(4, selectInput("frequenta_escola", "Frequenta Escola?", choices = c("", "Sim", "Não"))),
                       column(4, selectInput("escolaridade_familiar", "Escolaridade", choices = c("", "Fundamental", "Médio", "Superior", "Não informado")))
                     ),
                     
                     fluidRow(
                       column(4, selectInput("reside_com", "Reside com a assistida?", choices = c("", "Sim", "Não"))),
            ...
            tabPanel(
              "Dados de Residência", value = "residencia",
              selectInput(
            ...
            tabPanel(
              "Descrição da Fonte de Renda", value = "renda",
              selectInput(
        ...
        column(
          width = 2,  # antes era 3
          div(
            style = "background-color: #f9f9f9; border-left: 1px solid #ccc; padding: 15px; height: 100%;",
            h4("🗂️ Progresso"),
            uiOutput("painel_status")
          )
        )
...
server <- function(input, output, session) {
  ...
  campos_obrigatorios <- list(
    dados_iniciais = c("campo_inicial_1", "campo_inicial_2"),
  ...
  observeEvent(input$nova_ficha, {
    for (nome in names(abas_concluidas)) {
      abas_concluidas[[nome]] <- FALSE
    }
  })
  ...
  observe({
    if (input$rede == "CRM (Centro de Referência da Mulher)") {
      abas_concluidas$rede <- !is.null(input$profissional_crm) && input$profissional_crm != "Selecione"
    } else if (input$rede == "Casa de Mainha") {
      abas_concluidas$rede <- !is.null(input$profissional_mainha) && input$profissional_mainha != "Selecione"
    } else if (input$rede == "Casa Abrigo") {
      abas_concluidas$rede <- !is.null(input$profissional_abrigo) && input$profissional_abrigo != "Selecione"
    } else if (input$rede == "SEMMU Até Você") {
      abas_concluidas$rede <- !is.null(input$profissional_movel) && input$profissional_movel != "Selecione" &&
  ...
  observe({
    ...
    naturalidade_outros_ok <- input$naturalidade != "Outros" || 
      (!is.null(input$naturalidade_outros) && input$naturalidade_outros != "")
    
    uf_outros_ok <- input$uf != "Outros" || 
      (!is.null(input$uf_outros) && input$uf_outros != "")
    
    atividade_outros_ok <- input$atividade_laboral != "Outros" || 
      (!is.null(input$atividade_outros) && input$atividade_outros != "")
    
    deficiencia_outros_ok <- input$deficiencia != "Outros" || 
      (!is.null(input$deficiencia_outros) && input$deficiencia_outros != "")
    
    campos_ok <- campos_basicos && naturalidade_outros_ok && uf_outros_ok &&
      atividade_outros_ok && deficiencia_outros_ok
    
    abas_concluidas$notificacao <- campos_ok
  })
  
  # 7. Verificação condicional para aba "Dados Familiares"
  observe({
    abas_concluidas$familia <- !is.null(input$tabela_familia_rows_selected) || !is.null(input$nome_familiar)
  })
  ...
  output$abas_ui <- renderUI({
    tabsetPanel(
      id = "abas",
      tabPanel(
        title = if (abas_concluidas$dados_iniciais) "✔️ Dados Iniciais de Cadastro" else "Dados Iniciais de Cadastro",
        value = "dados_iniciais",
        ... # conteúdo da aba
      ),
      # repetir para as demais abas
    )
  })
  
  absolutePanel(
    top = 100, right = 20, width = 250,
    draggable = FALSE, fixed = TRUE,
    style = "background-color: #f9f9f9; border: 1px solid #ccc; padding: 15px; box-shadow: 2px 2px 5px rgba(0,0,0,0.1);",
  ...
  output$painel_status <- renderUI({
    abas <- list(
      dados_iniciais = "Dados Iniciais de Cadastro",
    ...
    lapply(names(abas), function(nome) {
      concluido <- abas_concluidas[[nome]]
      classe <- if (concluido) "concluido" else "pendente"
      icone <- if (concluido) "✔️" else "❌"
      
      div(class = paste("status-card", classe),
          span(class = "status-icon", icone),
          span(abas[[nome]])
      )
    })
  })
